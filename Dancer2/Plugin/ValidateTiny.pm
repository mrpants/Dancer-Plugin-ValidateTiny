package Dancer2::Plugin::ValidateTiny;
...
register validator => sub
{
	...
	my $rules = _load_rules($dsl,$rules_file);
...
sub _load_rules
{
	...
	$rules_file = $dsl->setting('appdir') . '/' . $settings->{rules_dir} . "/" . $rules_file;
...
Dancer2::Plugin::ValidateTiny - Validate::Tiny Dancer2 plugin.
...
Easy and cool validating data with L<Validate::Tiny> module:
    ...
    use Dancer2::Plugin::ValidateTiny;
...
Simple Dancer2 plugin for use L<Validate::Tiny> module.

It provides simple use for L<Validate::Tiny> way of validating user input with
Dancer2 applications with some additions and modifications, such as separate files
...
In your Dancer2 application directory you need to create sub-directory for rule files
...
Directory, where you will store your rule files. Plugin looking it in your Dancer2
application root.
